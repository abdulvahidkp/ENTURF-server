...
module.exports = {
    ...
    if (rejectUpdate) {
      if (image) {
        vms
          .findOneAndUpdate(
            { _id: id },
            { $set: { name, image, reason: "", status: "pending" } }
          )
          .then((response) => {
            res.status(200).json(response);
          });
        ...
        vms
          .findOneAndUpdate(
            { _id: id },
            { $set: { name, reason: "", status: "pending" } }
          )
          .then((response) => {
            res.status(200).json(response);
          });
  ...
  getLanding: async (req, res) => {
    try {
      ...
      const quickBookTurfs = await turfs.find({
        vmId: req._id,
        approved: true,
        isBlocked: false,
        vmIsBlocked: false,
      });
      const latestBooking = await bookings.aggregate([
        { $match: { refund: "not processed" } },
        {
          $lookup: {
            from: "users",
            localField: "userId",
        ...
        { $unwind: "$turfDetails" },
        { $match: { "turfDetails.vmId": new ObjectId(req._id) } },
        {
          $project: {
            _id: 1,
