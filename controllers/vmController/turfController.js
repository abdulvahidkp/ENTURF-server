...
module.exports = {
  ...
  addTurf: async (req, res) => {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "venueName, mobile, district, place, actualPrice, discountPercentage, description, image, document, slots, sportFacility, lat, lng - fields required",
      });
    turfs
      .create({ vmId: req._id, ...req.body })
      .then((response) => {
  ...
  changeBlock: async (req, res) => {
    ...
    await turfs
      .updateOne({ _id: id }, [
        { $set: { vmIsBlocked: { $eq: [false, "$vmIsBlocked"] } } },
      ])
      .then((response) => {
        res.sendStatus(200);
      })
